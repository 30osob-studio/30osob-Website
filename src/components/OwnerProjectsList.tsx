...
export default function OwnerProjectsList(): JSX.Element {
  ...
  return (
    <div className="w-full bg-[rgba(6,18,28,1)] relative py-12 px-12">
      <Carousel opts={{ align: "center" }} className="w-full">
        <CarouselContent className="gap-6">
          {repos.map((repo, index) => (
            <CarouselItem
              key={repo.name}
              className={`basis-auto shrink-0 ${index === 0 ? "ml-12" : ""} ${
                index === repos.length - 1 ? "mr-12" : ""
              }`}
            >
              <Project repo={repo} />
            </CarouselItem>
